...
var KTDatatableRemoteAjaxDemo = function() {
    ...
    // basic demo
    var demo = function() {
        var datatable = $('#kt_datatable_taskfilm').KTDatatable({
            // datasource definition
            data: {
                type: 'remote',
                source: {
                    read: {
                        url: `/${dashboard}/taskfilm/ajax/`,
                    },
                },
                pageSize: 10,
                serverPaging: true,
            ...
            columns: [{
                ...
                template: function(row) {
                    let count = 0;
                    if(row.Tasks.length > 0) count = row.Tasks.length;
                    return `<a href="/${dashboard}/task/?taskfilm=${row.url}" target="_blank" class="btn btn-outline-primary">${count}</a>`;
                },
            }, {
                field: 'updatedAt',
                title: 'Hành Động',
                ...
                overflow: 'visible',
                template: function(row) {
                    return `<a data-type="taskfilm" data-id="${row.id}" class="btn btn-icon btn-light btn-hover-primary btn-sm btn-edit" title="Sửa">
